                        ...
                        <div class="col-12 col-md-auto ">
                            <div class="">
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('vessels') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 ">
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/vessels/add`" class="full-width">
            ...
            <div class="container-fluid">
                <div class="row q-col-gutter-x-md">
                    <div class="col comp-grid">
                        <div>
                            <q-chip v-if="searchText" icon="las la-search" removable @remove="searchText='';$route.query.search=''">
                            ...
                            <div>
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="las la-angle-left" class="text-capitalize" :label="$route.query.tag" to="/vessels"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.VesselsTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/vessels/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        ...
                                                        <q-td key="btnactions" :props="props">
                                                            <div class="row q-col-gutter-xs justify-end">
                                                                <q-btn icon="las la-bars" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/vessels/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="las la-eye"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/vessels/edit/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="positive"  size="sm" name="las la-edit"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('edit') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple @click="deleteItem(props.row.id)">
                                                                                <q-item-section>
                                                                                    <q-icon color="negative"  size="sm" name="las la-times"></q-icon>
                                                                                </q-item-section>
                                                    ...
                                                    <div class="q-pa-sm" v-show="!loading">
                                                        <div class="row justify-between">
                                                            <div class="row q-col-gutter-md">
                                                                <div>
                                                                    <q-btn    :rounded="false"  no-caps  unelevated   color="negative" padding="xs" @click="deleteItem(selectedItems)" v-if="selectedItems.length" icon="las la-trash" class="q-my-xs" :title="$t('delete_selected')"></q-btn>
                                                                </div>
                                                            </div>
...
<script>
	...
	export default {
		...
		computed: {
			pageTitle:{
				get: function () {
					...
					return this.$t('vessels')
				}
			},
			records: {
				get: function () {
					return this.$store.getters["vessels/records"];
				},
				set: function (value) {
					this.$store.commit("vessels/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("vessels", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
